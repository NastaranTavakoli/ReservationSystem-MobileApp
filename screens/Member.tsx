...
import {
  ...
  getReservationById,
  requestUpdate,
} from '../services/memberReservations';
...
export const MemberScreen: React.FC<MemberScreenProps> = ({ navigation }) => {
    ...
    getAllReservations()
      .then((data: Array<string | number>) => {
        ...
        const future = data.filter(
          (d: any) => new Date(d.startTime) > new Date()
        );
    ...
    if (props != null && new Date(props.startTime) > new Date()) {
      return (
        ...
        >
          ...
          <TextInput
            ...
            value={notes}
          />
          <Button mode='outlined' onPress={() => requestOnPress('update')}>
            Request Update
          </Button>
          <Button mode='outlined' onPress={() => requestOnPress('cancel')}>
      ...
      return (
        <Dialog
          ...
          title={`Reservation for ${props.restaurantName}`}
        >
          <Paragraph>{`Date: ${moment(props.startTime).format(
            'Do MMM YYYY, h:mm a'
    ...
    if (props.length != 0) {
      return props.map((r: any) => (
        <List.Item
          key={r.id}
          right={() => (
            <Button onPress={() => detailsOnPress(r.id)}>Details</Button>
          )}
          title={moment(r.startTime).format('Do MMM YYYY, h:mm a')}
          description={`Venue: ${r.restaurant}\nState: ${r.status}`}
        />
      ));
    } else {
  ...
  const requestOnPress = (button: string) => {
    const details = {
      ...
      notes: notes,
      phone: phone,
      requestType: button == 'update' ? 'update' : 'cancel',
      startTime: startTime,
    };
  ...
  return (
    <View style={styles.container}>
      <ScrollView>
        <Button
          mode='outlined'
          onPress={() => console.log('Book Now button pressed.')}
        >
          Book Now
        </Button>
        <List.Accordion
        ...
        >
          {loading ? (
            ...
            renderReservations(nextReservations)
          )}
        </List.Accordion>
